...
const Navbar = () => {
  ...
  return (
    <motion.nav style={{ opacity: 1 }} className={`fixed top-0 w-full z-50 px-8 py-4 ${isDarkMode ? "bg-gray-950/80" : "bg-gray-50/80"} backdrop-blur-md border-b ${isDarkMode ? "border-gray-800" : "border-gray-200"}`}>
        <div className='max-w-7xl mx-auto flex items-center justify-between'>
            <motion.div whileHover={{ scale: 1.05}} className='flex items-center space-x-2 cursor-pointer' onClick={() => scrollToSection('accueil')}>
                <Code2 size={24} className='text-blue-500' />{" "}
                <span className={`text-lg ml-1 font-semibold ${isDarkMode ? 'text-white' : 'text-black' }`}>Ben likes to code</span>
            </motion.div>
            ...
            <div className='hidden md:flex items-center space-x-8 cursor-pointer'>
                ...
                <motion.button
                    ...
                    className={`p-2 rounded-full transition-colors ${isDarkMode ? "text-gray-400 hover:text-white hover:bg-gray-800" : "text-gray-600  hover:bg-gray-200"}`}
                >
                    {isDarkMode ? <Sun size={18} className='cursor-pointer' /> : <Moon size={18} className='cursor-pointer' />}
                </motion.button>
            </div>
            ...
            <div className='md:hidden flex items-center space-x-4'>
                ...
                <motion.button whileHover={{ scale: 1.05}} whileTap={{ scale: 0.95}} onClick={()=> setIsMenuOpen(!isMenuOpen)} className={`p-2 rounded-full transition-colors ${isDarkMode ? "text-gray-400 hover:text-white hover:bg-gray-800" : "text-gray-600 hover:text-gray-900 hover:bg-gray-200" } `} >
                    {isMenuOpen ? <X size={20} className='cursor-pointer' /> : <Menu size={20} className='cursor-pointer' /> }
                </motion.button>
            </div>
        </div>
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0}} exit={{ opacity: 0, y: -20}} className={`md:hidden mt-4 p-4 rounded-lg ${isDarkMode ? 'bg-gray-900' : "bg-white"} border ${isDarkMode ? "border-gray-800" : "border-gray-200" } `}>
                    {["Accueil", "Compétences", "Projets", "A propos", "Contact"].map((item) => (
                        <motion.button
                            ...
                            whileHover={{ x: 5 }}
                            onClick={() => scrollToSection(item.toLowerCase())}
                            className={`block w-full text-left py-2 text-sm uppercase tracking-wider ${isDarkMode ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-gray-900" }`}
                        >
                            {item}
                        </motion.button>
                    ))}
