  ...
  return (
    <section className={`py-20 px-6 ${isDarkMode ? 'bg-gray-950 text-white' : 'bg-gray-50 text-gray-900'} relative overflow-hidden`} ref={sectionRef}>
      ...
      <motion.div style={{ y }}  className='absolute inset-0 overflow-hidden'>
        ...
        <div className={`absolute bottom-36 left-1/4 w-64 h-64 rounded-full blur-3xl opacity-5 ${isDarkMode ? "bg-purple-500" : "bg-purple-400"}`} />
      </motion.div>

      <div className='max-w-6xl mx-auto relative z-10'>
        {/* Section Header */}
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className='text-center mb-16'>
          ...
          <motion.h2 variants={itemVariants} className='text-2xl md:text-2xl font-light mb-3'>
            Compétences & <span className='text-blue-500 font-medium'>Technologies</span>
          </motion.h2>

          
          <motion.p variants={itemVariants} className={`text-lg ${isDarkMode ? "text-gray-400" : "text-gray-600"} max-w-2xl mx-auto font-light`}>
            Une boîte à outils complète pour la création d'applications web modernes et évolutives de la conception au déploiement.
          </motion.p>

        </motion.div>

        {/* Skills Grid */}
        <motion.div className='grid md:grid-cols-2 gap-8 lg:gap-12' variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"}>
                ...
                {category.skills.map((skill, skillIndex) => (
                  <div className='group' key={skill.name}>
                    <div className='flex justify-between items-center mb-2'>
                      ...
                      <span className={`text-xs ${isDarkMode ? "text-gray-600" : "text-gray-700"}`}>
                        {skill.level}%
                      </span>
                    </div>
        ...
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className='mt-16'>
          <motion.div className='text-center mb-6' variants={itemVariants}>
            <h3 className='text-xl uppercase tracking-widest font-semibold'>Je travaille aussi avec</h3>
          </motion.div>

          <motion.div className='flex flex-wrap justify-center gap-6' variants={itemVariants}>
            {TECH_STACK.map((tech, index) => (
              <motion.div key={tech} whileHover={{ y: -2, scale: 1.05 }} className={`px-4 py-2 text-sm rounded-full border transition-all duration-100 cursor-pointer ${isDarkMode ? "bg-gray-900 border-gray-700 text-gray-300 hover:border-gray-600" : "bg-white border-gray-300 text-gray-700 hover:border-gray-400"}`}>
                {tech}
              </motion.div>
            ))}
          </motion.div>
        ...
        {/* Stats */}
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className='mt-16 grid grid-cols-2 md:grid-cols-4 gap-8'>
